...
let perform_round acc =
  ...
  let dir' = match piece_state with
    ...
    | Flagged -> turn_back dir in
  let piece_state' = match piece_state with
    | Clean -> Weakened
    | Weakened -> Infected
    | Infected -> Flagged
    | Flagged -> Clean in
  let board' = Board.add pos piece_state' board in
  let pos' = move_forward pos dir' in
  let infection_count' = if piece_state' = Infected then infection_count + 1 else infection_count in
...
let () =
  ...
  let board', _, _, c = better_repeat perform_round (board, {x=0;y=0}, North, 0) 10000000 in
